...
template <class TileShape_MNK_, class ClusterShape_, class Element_,
...
struct CollectiveEpilogueFwd {
  ...
  static constexpr bool Use_TMA_O =
      ArchTag::kMinComputeCapability >= 90 && !Varlen && Use_smem && !PackGQA;
  ...
  static_assert(kHeadDim % kGmemElemsPerStore == 0,
  ...
  using SmemLayoutOTMA = decltype(tile_to_shape(SmemLayoutAtomOTMA{},
  ...
  using TMA_O = std::conditional_t<
      ...
      decltype(make_tma_copy(
          ...
          make_tensor(make_gmem_ptr(static_cast<Element *>(nullptr)), ShapeO{},
                      StrideO{}),
          SmemLayoutOTMA{}, select<0, 2>(TileShape_MNK{}), _1{})), // no mcast
                                                                   // for O
      std::nullptr_t>;
  ...
  static Params to_underlying_arguments(Arguments const &args) {
    ...
    TMA_O tma_store_O = [&] {
      if constexpr (Use_TMA_O) {
        return make_tma_copy(GmemTiledCopyOTMA{}, mO, SmemLayoutO{},
                             select<0, 2>(TileShape_MNK{}), _1{}); // no mcast
      } else {
        return nullptr;
      }
    }();
    ...
    return {args.ptr_O,        args.shape_O,
            args.stride_O,     shape_O_packed,
            stride_O_packed,   args.ptr_LSE,
            args.stride_LSE,   shape_LSE_packed,
            stride_LSE_packed, cutlass::FastDivmod(qhead_per_khead),
            tma_store_O,       args.cu_seqlens,
            args.seqused};
  }
  ...
  template <typename SharedStorage, typename FrgTensorO, typename FrgTensorLSE,
            typename TiledMma>
  CUTLASS_DEVICE void
  store(Params const &params, FrgTensorO const &tOrO, FrgTensorLSE const &lse,
    ...
    if constexpr (FP8PermuteCol &&
                  (sizeof(Element) == 2 || sizeof(Element) == 4)) {
...
#pragma unroll
    ...
    using PackGQAt =
        flash::PackGQAManager<get<0>(TileShape_MNK{}), get<2>(TileShape_MNK{}),
    ...
    Tensor mLSE = make_tensor(
        ...
        params.stride_LSE_packed)(_, bidh, !is_varlen ? bidb : 0, split_idx);
...
#pragma unroll
      ...
      PackGQAt::store_LSE(mLSE, lse, tiled_mma, params.qhead_per_khead_divmod,
    ...
    if constexpr (Use_TMA_O) {
      ...
      Tensor gO = local_tile(mO, select<0, 2>(TileShape_MNK{}),
...
#pragma unroll
      ...
      Tensor mO = make_tensor(
          ...
          params.stride_O_packed)(_, _, bidh, !is_varlen ? bidb : 0, split_idx);
      Tensor gO = local_tile(mO, select<0, 2>(TileShape_MNK{}),
                             make_coord(m_block, _0{})); // (M, K)
...
#pragma unroll
          ...
          // same row
          PackGQAt::store_O(mO, tOrO, params.qhead_per_khead_divmod, thread_idx,
                            seqlen_o, m_block);
        }
      } else {
...
#pragma unroll
          ...
          PackGQAt::store_O_direct(mO, tOrO_out, tiled_mma,
    ...
    if (thread_idx < kBlockM) {
      ...
      if constexpr (!PackGQA) {
        ...
        if (row < seqlen_o * qhead_per_khead) {
          ...
          // mLSE shape shape ((qhead_per_khead, seqlen_q)) and it's unhappy
          // with just 1 "make_coord"
...
#pragma unroll
      ...
      using PackGQAt = flash::PackGQAManager<get<0>(TileShape_MNK{}),
                                             get<2>(TileShape_MNK{}),
                                             NumEpilogueThreads, Element>;
      Tensor tOrO = make_tensor<Element>(make_shape(
          Shape<_1, Int<kGmemElemsPerStore>>{}, size<1>(tOcO), size<2>(tOcO)));
      cute::clear(tOrO);
      PackGQAt::store_O(mO, tOrO, params.qhead_per_khead_divmod, thread_idx,
                        seqlen_o, m_block);
    }
