...
template <int Arch, int kHeadDim, int ClusterM, typename Element,
          typename ElementOut, bool Is_causal, bool Is_local, bool Has_softcap,
          bool Varlen, bool PagedKV, bool AppendKV, bool PackGQA, bool Split,
          bool V_colmajor>
void run_flash_fwd(Flash_fwd_params &params, cudaStream_t stream) {
  ...
  static constexpr std::tuple<int, int, bool, bool> kBlockMN_RS_IntraWGOverlap =
      tile_size_fwd_sm90(kHeadDim, Is_causal, Is_local,
                         sizeof(Element) /*element_size*/, V_colmajor, PagedKV,
                         Has_softcap);
  static constexpr std::tuple<int, int, int, int, bool>
      kBlockMN_kNWarps_Stages_RS =
          tile_size_fwd_sm8x(Arch == 86 || Arch == 89, kHeadDim, Is_causal,
                             Is_local, sizeof(Element) /*element_size*/,
                             PagedKV, Varlen && Split, Has_softcap, AppendKV);
  ...
  using CollectiveMainloop = std::conditional_t<
      ...
      flash::CollectiveMainloopFwdSm90<
          kStages, ClusterShape, TileShape_MNK, Element, float,
          cutlass::arch::Sm90, Is_causal, Is_local, Has_softcap, Varlen,
          PagedKV, AppendKV, Mma1_is_RS, IntraWGOverlap, PackGQA, Split,
          V_colmajor>,
      flash::CollectiveMainloopFwdSm80<
          kNWarps, kStages, Q_in_regs, TileShape_MNK, Element, float,
          cutlass::arch::Sm80, Is_causal, Is_local, Has_softcap, Varlen,
          PagedKV, AppendKV, PackGQA, Split>>;
  using CollectiveEpilogue =
      flash::CollectiveEpilogueFwd<TileShape_MNK, ClusterShape, ElementOut,
                                   ArchTag, CollectiveMainloop::NumMmaThreads,
                                   Varlen, PackGQA, FP8_TransposeV>;
  ...
  using SchedulerPersistent = std::conditional_t<
      ...
      flash::VarlenDynamicPersistentTileScheduler<
          kBlockM, CollectiveMainloop::NumMmaThreads, NumProducerThreads, Split,
  ...
  using Scheduler =
      std::conditional_t<Arch >= 90
                             ? (Split && !Varlen)
                             : !((Is_causal && !Varlen) || (Varlen && Split)),
                         SchedulerSingleTile, SchedulerPersistent>;
  ...
  typename CollectiveMainloop::Arguments mainloop_args{
      ...
      {!PagedKV ? (!is_varlen_k ? params.seqlen_k : params.total_k)
                : params.page_size,
      ...
      {params.kv_batch_idx ? params.b_k : params.b,
       !PagedKV ? 0 : params.seqlen_k / params.page_size}, // shape_page_table
  ...
  typename CollectiveEpilogue::Arguments epilogue_args{
      ...
      {!Split ? params.o_row_stride : params.oaccum_row_stride, _1{},
       !Split ? params.o_head_stride : params.oaccum_head_stride,
       !is_varlen_q
           ? (!Split ? params.o_batch_stride : params.oaccum_batch_stride)
           : 0,
       !Split ? 0 : params.oaccum_split_stride}, // stride_O
      static_cast<float *>(!Split ? params.softmax_lse_ptr
                                  : params.softmax_lseaccum_ptr),
      {_1{}, seqlen_q, !is_varlen_q ? params.h * seqlen_q : 0,
       !Split ? 0 : params.h * seqlen_q * batch_q}, // stride_LSE
      params.h_k,
      params.cu_seqlens_q,
      params.seqused_q};
...
template <int Arch, typename T, int kHeadDim, bool Split, bool PagedKV,
...
void run_mha_fwd_(Flash_fwd_params &params, cudaStream_t stream) {
  ...
  using T_out = std::conditional_t<
      !Split, std::conditional_t<!Is_FP8, T, cutlass::bfloat16_t>, float>;
  CAUSAL_LOCAL_SWITCH(
          ...
          VARLEN_SWITCH(
              ...
              Varlen, [&] {
                ...
                static constexpr bool Enable_cluster =
                    ...
                    !Is_causal && !Is_local && !Split && !PagedKV && !Varlen;
                APPENDKV_SWITCH(params.knew_ptr, AppendKV, [&] {
                  ...
                  CLUSTER_SWITCH(
                      ...
                      Use_cluster, [&] {
                        ...
                        run_flash_fwd<Arch, kHeadDim, ClusterM, T, T_out,
                                      Is_causal, Is_local, Has_softcap, Varlen,
                                      PagedKV, AppendKV && Varlen, PackGQA,
