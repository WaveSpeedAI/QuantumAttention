...
struct CollectiveMainloopBwdSm80 {
  ...
  using SeqlenInfo_t =
      flash::SeqlenInfoQK<Varlen, CUTE_STATIC_V(get<0>(TileShape_MNK{}))>;
  ...
  CUTLASS_DEVICE
  cute::tuple<int, int> get_m_block_min_max(Params const &params,
      ...
      if (n_block >= cute::ceil_div(params.sink_token_length, kBlockN)) {
        m_block_max = std::min(
            m_block_max, cute::ceil_div((n_block + 1) * kBlockN + seqlen_q -
                                            seqlen_k + params.window_size_left,
                                        kBlockM));
      }
    }
    int m_block_min = 0;
    if constexpr (Is_causal || Is_local) {
      m_block_min =
          std::max(m_block_min, (n_block * kBlockN + seqlen_q - seqlen_k -
  ...
  CUTLASS_DEVICE bool mma(Params const &params, FrgTensordKV &tdKrdK,
    ...
    SeqlenInfo_t seqlen_info{bidb,
                             ...
                             params.seqused_q,
                             params.seqused_k};
    auto m_block_min_max =
        get_m_block_min_max(params, seqlen_info, n_block, bidb);
    ...
    Tensor mdO =
        make_tensor(make_gmem_ptr(params.ptr_dO), params.shape_Q,
