...
template <int Stages, class ClusterShape_, class TileShape_MNK_, class Element_,
          ...
          bool Has_softcap_, bool Varlen_, bool PagedKV_, bool AppendKV_,
          bool Mma1_is_RS, bool IntraWGOverlap, bool PackGQA_, bool Split_,
          bool V_colmajor_>
struct CollectiveMainloopFwdSm90 {
  ...
  static_assert(!(!Mma1_is_RS && !IntraWGOverlap),
                "Mma1 must be RS if IntraWGOverlap is enabled");
  static_assert(!(!Mma1_is_RS && Is_FP8), "Mma1 must be RS if FP8");
  static_assert(!(!Mma1_is_RS && Transpose_V),
  ...
  using SmemLayoutAtomVt =
      decltype(cutlass::gemm::collective::detail::ss_smem_selector<
               TmaMajorV, Element, decltype(cute::get<2>(TileShape_MNK{})),
  ...
  using SmemLayoutAtomVtMma =
      decltype(cutlass::gemm::collective::detail::ss_smem_selector<
               MmaMajorV, Element, decltype(cute::get<2>(TileShape_MNK{})),
               decltype(cute::get<1>(TileShape_MNK{}))>());
  ...
  using LDSM_thread_shape =
      std::conditional_t<kHeadDim_multiple_64, Shape<_32, _4, _1, _1>,
                         Shape<_16, _4, _1, _2>>;
  using LDSM_thread_stride =
      std::conditional_t<kHeadDim_multiple_64, Stride<_4, _1, _0, _0>,
                         Stride<_4, _1, _0, _64>>;
  ...
  using STSM_thread_shape =
      std::conditional_t<kHeadDim_multiple_64, Shape<_8, _4, _4, _1>,
                         Shape<_8, _4, _2, _2>>;
  using STSM_thread_stride =
      std::conditional_t<kHeadDim_multiple_64, Stride<_4, _1, _32, _0>,
  ...
  using TMA_V = decltype(make_tma_copy(
      ...
      take<0, 2>(SmemLayoutVt{}), select<2, 1>(TileShape_MNK{}),
  ...
  using SmemP_t = std::conditional_t<
      Mma1_is_RS, cute::array<Element, 0>,
  ...
  using TensorStorageNoTranspose =
      std::conditional_t<Mma1_is_RS, TensorStorageWithoutPNoTranspose,
  ...
  static Params to_underlying_arguments(Arguments const &args) {
    ...
    TMA_K tma_load_K = make_tma_copy_B_sm90(
        ...
        ClusterShape{}); // mcast along M mode for this N load, if any
    Tensor mV =
        make_tensor(make_gmem_ptr(args.ptr_V), select<1, 0, 2, 3>(args.shape_K),
    ...
    Tensor mVnew = make_tensor(make_gmem_ptr(args.ptr_V_new),
                               select<1, 0, 2, 3>(args.shape_K_new),
  ...
  CUTLASS_DEVICE
  cute::tuple<int, int> get_n_block_min_max(Params const &params,
                                            SeqlenInfo_t const &seqlen_info,
                                            int m_block, int bidb,
                                            int split_idx = 0,
                                            int num_splits = 1) {
    static constexpr int kBlockM = get<0>(TileShape_MNK{});
    static constexpr int kBlockN = get<1>(TileShape_MNK{});
    ...
    if constexpr (Is_causal || Is_local) {
      ...
      if (PackGQA) {
        m_idx_max = params.qhead_per_khead_divmod.divide(m_idx_max - 1) + 1;
      }
      n_block_max =
          std::min(n_block_max, cute::ceil_div(m_idx_max + seqlen_k - seqlen_q +
    ...
    if constexpr (Is_local) {
      ...
      if (PackGQA) {
        m_idx_min = params.qhead_per_khead_divmod.divide(m_idx_min);
      }
      n_block_min = std::max(
          int(0), (m_idx_min + seqlen_k - seqlen_q - params.window_size_left) /
                      kBlockN);
    }
    ...
    if constexpr (Split) {
      int num_n_blocks_per_split =
          n_block_max <= n_block_min
              ? 0
              : cute::ceil_div(n_block_max - n_block_min, num_splits);
  ...
  load(Params const &params, MainloopPipelineK pipeline_k,
    ...
    auto [n_block_min, n_block_max] = get_n_block_min_max(
        params, seqlen_info, m_block, bidb, split_idx, params.num_splits);
    ...
    Tensor mVt_TMA = params.tma_load_V.get_tma_tensor(select<1, 0, 2, 3>(
        params.shape_K))(_, _, bidh_kv, !is_varlen_k ? bidb_kv : 0);
    ...
    Tensor gVt_TMA = local_tile(
        ...
        select<2, 1>(TileShape_MNK{}), make_coord(_0{}, _)); // (K, N, _)
    ...
    PagedKVManager_t paged_kv_manager(
        ...
        params.stride_V, params.page_size_divmod, bidb_kv, bidh_kv, thread_idx,
        seqlen_info.seqlen_k, seqlen_info.leftpad_k);
...
#pragma unroll 1
      for (n_block = std::min(n_block, n_block_sink_max - 1); n_block >= 0;
           --n_block) {
        PipelineState smem_pipe_write_v =
            smem_pipe_write; // copy the state, write_v is always 1 step behind
        ++smem_pipe_write;
  ...
  mma(Params const &params, MainloopPipelineK pipeline_k,
    ...
    if constexpr (!AppendKV) {
      ...
      if (get<1>(params.shape_rotary) > 0) { // Apply rotary to Q
        int const offset_rotary =
            seqlen_info.seqlen_k_og + seqlen_info.leftpad_k;
    ...
    // TODO: check the case where n_block_max <= n_block_min but there are sink
    // tokens
    if constexpr (IntraWGOverlap) {
      Tensor tSrS =
      ...
      Tensor tOrP_acc = make_tensor(
          tSrS.data(),
          flash::convert_layout_acc_Aregs<TiledMma1>(tSrS.layout()));
      ...
      auto fwd_step = [&](int const n_block, auto mask_fn,
        ...
        flash::gemm</*zero_init=*/false, /*wg_wait=*/-1>(
            tiled_mma1, cute::conditional_return<Mma1_is_RS>(tOrP, tOsP),
      ...
      if constexpr (Is_causal || Is_local) { // Separate iterations with causal
        ...
        int const n_block_min_causal_local_mask =
            std::max(n_block_min, (m_idx_min + seqlen_k - seqlen_q +
                                   params.window_size_right) /
                                      kBlockN);
#pragma unroll 1
      ...
      int const m_idx_max = !PackGQA ? (m_block + 1) * kBlockM
                                     : params.qhead_per_khead_divmod.divide(
                                           (m_block + 1) * kBlockM - 1) +
                                           1;
      int const n_block_min_before_local_mask =
          !Is_local ? n_block_min
                    : std::max(n_block_min,
                               cute::ceil_div(m_idx_max + seqlen_k - seqlen_q -
...
#pragma unroll 1
        ...
        // Disable sink token code for now
        // int n_block_sink_max = cute::ceil_div(params.sink_token_length,
        // kBlockN); #pragma unroll 1 for (n_block = std::min(n_block,
        // n_block_sink_max - 1); n_block >= 0; --n_block) {
        //     fwd_step(n_block, local_mask_fn, cute::bool_constant<Is_local>{}
        //     /*check_inf*/);
        // }
      }
      // Tell producers that smem_q is ready
...
#pragma unroll 1
        ...
        // kBlockN); #pragma unroll 1 for (n_block = std::min(n_block,
        // n_block_sink_max - 1); n_block >= 0; --n_block) {
        //     fwd_step(n_block, local_mask_fn, cute::false_type{}
        //     /*is_first_iter*/, cute::bool_constant<Is_local>{}
        //     /*check_inf*/);
        // }
      }
      warp_scheduler_barrier_arrive();
  ...
  CUTLASS_DEVICE
  cute::tuple<int, int>
  get_n_block_k_new_min_max(Params const &params,
                            SeqlenInfo_t const &seqlen_info, int m_block,
                            int bidb, int split_idx = 0, int num_splits = 1) {
    static constexpr int kBlockN = get<1>(TileShape_MNK{});
    auto [n_block_min, n_block_max] = get_n_block_min_max(
        params, seqlen_info, m_block, bidb, split_idx, num_splits);
  ...
  load_kv_new(Params const &params, MainloopPipelineKVNew pipeline_k_new,
    ...
    auto [m_block, bidh, bidb, split_idx] = block_coord;
    auto [n_block_new_min, n_block_new_max] = get_n_block_k_new_min_max(
        params, seqlen_info, m_block, bidb, split_idx, params.num_splits);
    ...
    Tensor mVnewt_TMA = params.tma_load_V_new.get_tma_tensor(select<1, 0, 2, 3>(
        params.shape_K_new))(_, _, bidh_kv, !is_varlen_k_new ? bidb : 0);
...
#pragma unroll 1
  ...
  store_kv_new(Params const &params, MainloopPipelineKVNew pipeline_k_new,
    ...
    using PagedKVManager_t =
        PagedKVManager<get<1>(TileShape_MNK{}), get<2>(TileShape_MNK{}),
                       NumMmaThreads, Element, true /*KV_Same_Iter*/,
                       2 /*LoadsPerRow_LB*/>;
    PagedKVManager_t paged_kv_manager(
        params.ptr_pagetable, params.shape_pagetable, params.stride_pagetable,
        params.ptr_K, params.shape_K, params.stride_K, params.ptr_V,
        params.stride_V, params.page_size_divmod, bidb_kv, bidh_kv, thread_idx,
        seqlen_k_new, offset_k
