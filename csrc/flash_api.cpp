...
void set_params_dgrad(Flash_bwd_params &params,
                      ...
                      at::Tensor dv, void *cu_seqlens_q_d, void *cu_seqlens_k_d,
                      void *seqused_q, void *seqused_k, void *dq_accum_d,
                      ...
                      int window_size_right, const float softcap = 0.f,
  ...
  set_params_fprop(params, b, seqlen_q, seqlen_k, seqlen_q_rounded,
                   ...
                   window_size_right, softcap, sm_margin);
    ...
    SPLIT_SWITCH(params.num_splits > 1, Split, [&] {
      PAGEDKV_SWITCH(params.page_table, PagedKV, [&] {
        PACKGQA_SWITCH(params.pack_gqa, PackGQA_, [&] {
          // Always enable PackGQA for Sm8x or PagedKV or Split to reduce
          // compilation
          static constexpr bool PackGQA =
              PackGQA_ || Arch < 90 || PagedKV || Split;
          SOFTCAP_SWITCH(params.softcap > 0.0, Has_softcap, [&] {
            if (!params.is_e4m3) {
              if (params.is_bf16) {
#ifndef FLASHATTENTION_DISABLE_HDIM64
                if (params.d <= 64) {
                  return run_mha_fwd_<Arch, cutlass::bfloat16_t, 64, Split,
                                      PagedKV, Has_softcap, PackGQA>(params,
                                                                     stream);
                }
#endif
#ifndef FLASHATTENTION_DISABLE_HDIM96
                if (params.d <= 96) {
                  return run_mha_fwd_<Arch, cutlass::bfloat16_t, 96, Split,
                                      PagedKV, Has_softcap, PackGQA>(params,
                                                                     stream);
                }
#endif
                ...
                if (params.d <= 128) {
                  return run_mha_fwd_<Arch, cutlass::bfloat16_t, 128, Split,
                                      PagedKV, Has_softcap, PackGQA>(params,
                                                                     stream);
                }
#endif
                ...
                if (params.d <= 192) {
                  return run_mha_fwd_<Arch, cutlass::bfloat16_t, 192, Split,
                                      PagedKV, Has_softcap, PackGQA>(params,
                                                                     stream);
                }
#endif
                ...
                if (params.d <= 256) {
                  return run_mha_fwd_<Arch, cutlass::bfloat16_t, 256, Split,
                                      PagedKV, Has_softcap, PackGQA>(params,
                                                                     stream);
                }
#endif
...
#ifndef FLASHATTENTION_DISABLE_HDIM64
                if (params.d <= 64) {
                  return run_mha_fwd_<Arch, cutlass::half_t, 64, Split, PagedKV,
                                      Has_softcap, PackGQA>(params, stream);
                }
#endif
#ifndef FLASHATTENTION_DISABLE_HDIM96
                if (params.d <= 96) {
                  return run_mha_fwd_<Arch, cutlass::half_t, 96, Split, PagedKV,
                                      Has_softcap, PackGQA>(params, stream);
                }
#endif
#ifndef FLASHATTENTION_DISABLE_HDIM128
                if (params.d <= 128) {
                  return run_mha_fwd_<Arch, cutlass::half_t, 128, Split,
                                      PagedKV, Has_softcap, PackGQA>(params,
                                                                     stream);
                }
#endif
#ifndef FLASHATTENTION_DISABLE_HDIM192
                if (params.d <= 192) {
                  return run_mha_fwd_<Arch, cutlass::half_t, 192, Split,
                                      PagedKV, Has_softcap, PackGQA>(params,
                                                                     stream);
                }
#endif
#ifndef FLASHATTENTION_DISABLE_HDIM256
                if (params.d <= 256) {
                  return run_mha_fwd_<Arch, cutlass::half_t, 256, Split,
                                      PagedKV, Has_softcap, PackGQA>(params,
                                                                     stream);
                }
#endif
...
#ifndef FLASHATTENTION_DISABLE_HDIM64
              if (params.d <= 64) {
                return run_mha_fwd_<90, cutlass::float_e4m3_t, 64, Split,
                                    PagedKV, Has_softcap, PackGQA>(params,
                                                                   stream);
              }
#endif
#ifndef FLASHATTENTION_DISABLE_HDIM96
              if (params.d <= 96) {
                return run_mha_fwd_<90, cutlass::float_e4m3_t, 96, Split,
                                    PagedKV, Has_softcap, PackGQA>(params,
                                                                   stream);
              }
#endif
#ifndef FLASHATTENTION_DISABLE_HDIM128
              if (params.d <= 128) {
                return run_mha_fwd_<90, cutlass::float_e4m3_t, 128, Split,
                                    PagedKV, Has_softcap, PackGQA>(params,
                                                                   stream);
              }
#endif
#ifndef FLASHATTENTION_DISABLE_HDIM192
              if (params.d <= 192) {
                return run_mha_fwd_<90, cutlass::float_e4m3_t, 192, Split,
                                    PagedKV, Has_softcap, PackGQA>(params,
                                                                   stream);
              }
#endif
#ifndef FLASHATTENTION_DISABLE_HDIM256
              if (params.d <= 256) {
                return run_mha_fwd_<90, cutlass::float_e4m3_t, 256, Split,
                                    PagedKV, Has_softcap, PackGQA>(params,
                                                                   stream);
              }
#endif
...
inline bool get_pack_gqa(Flash_fwd_params const &params) {
  ...
  // and binary size. Has little effect on speed.
  if (params.arch < 90 || params.page_table || params.num_splits > 1) {
    return true;
  }
  ...
  auto kBlockMN_kernel_args_sm90 = tile_size_fwd_sm90(
      params.d_rounded, params.is_causal, params.is_local,
      params.is_e4m3 ? 1 : 2 /*element_size*/, false /*v_colmajor*/,
      params.page_table, params.softcap > 0.f);
...
inline int get_num_splits(Flash_fwd_params const &params) {
  ...
  // params.page_table must already be set
  // This needs to match the kernel configs
  bool varlen = params.cu_seqlens_q || params.cu_seqlens_k ||
  ...
  return num_splits_heuristic(
      params.b * (!params.pack_gqa ? params.h : params.h_k) * num_m_blocks,
      params.num_sm, num_n_blocks, 128);
// return num_splits_heuristic(params.b * params.h_k * num_m_blocks, params.b *
// params.h_k,
//                             params.num_sm, num_n_blocks, 128,
//                             params.d_rounded);
#endif
...
std::vector<at::Tensor> mha_fwd(
    ...
    const at::Tensor
        &v, // (b_k, s_k, h_k, d) or (total_k, h_k, d) if there is cu_seqlens_k
            // or (num_pages, page_size, h_k, d) if there is page_table.
    std::optional<const at::Tensor>
        &k_new_, // (b, s_k_new, h_k, d) or (total_k_new, h_k, d) if there is
                 // cu_seqlens_k_new
    std::optional<const at::Tensor>
        &v_new_, // (b, s_k_new, h_k, d) or (total_k_new, h_k, d) if there is
                 // cu_seqlens_k_new
    std::optional<at::Tensor>
        &out_, // (b, s_q, h, d) or (total_q, h, d) if there is cu_seqlens_q
    ...
    float const softmax_scale, bool is_causal, int window_size_left,
    int window_size_right, int sink_token_length, float const softcap,
    bool const is_rotary_interleaved, // if true, rotary combines indices 0 & 1,
                                      // else indices 0 & rotary_dim / 2
    int num_splits, std::optional<bool> pack_gqa_, int const sm_margin) {
  ...
  params.pack_gqa =
      pack_gqa_.has_value() ? pack_gqa_.value() : get_pack_gqa(params);
  ...
  if (params.num_splits > 1) {
    ...
    if (!is_varlen_q) {
      out_accum = torch::empty(
          {params.num_splits, batch_size, num_heads, seqlen_q, head_size},
      ...
      out_accum =
          torch::empty({params.num_splits, num_heads, total_q, head_size},
                       opts.dtype(outaccum_type));
  ...
  bool const persistent_scheduler =
      params.arch >= 90 ? (((params.is_causal || params.is_local) &&
                            (params.num_splits == 1)) ||
                           is_varlen)
...
#endif
#ifdef FLASHATTENTION_DISABLE_PACKGQA
  TORCH_CHECK(!params.pack_gqa || params.arch < 90 || params.page_table ||
                  params.num_splits > 1,
  ...
  if (total_q > 0 && (total_k + params.total_knew) > 0 && num_heads_k > 0) {
    ...
    if (params.num_splits > 1) {
      ...
      // Varlen there. if (is_varlen_q && !seqused_q_.has_value()) {
      if (is_varlen_q) {
        params.b = 1;
...
#ifndef FLASHATTENTION_DISABLE_HDIM64
        if (params.d <= 64) {
          return run_mha_bwd_<Arch, cutlass::half_t, 64, Has_softcap>(params,
                                                                      stream);
        }
#endif
#ifndef FLASHATTENTION_DISABLE_HDIM96
        if (params.d <= 96) {
          return run_mha_bwd_<Arch, cutlass::half_t, 96, Has_softcap>(params,
                                                                      stream);
        }
#endif
#ifndef FLASHATTENTION_DISABLE_HDIM128
        if (params.d <= 128) {
          return run_mha_bwd_<Arch, cutlass::half_t, 128, Has_softcap>(params,
                                                                       stream);
        }
#endif
#ifndef FLASHATTENTION_DISABLE_HDIM192
        if (params.d <= 192) {
          return run_mha_bwd_<Arch, cutlass::half_t, 192, Has_softcap>(params,
                                                                       stream);
        }
#endif
#ifndef FLASHATTENTION_DISABLE_HDIM256
        if (params.d <= 256) {
          return run_mha_bwd_<Arch, cutlass::half_t, 256, Has_softcap>(params,
                                                                       stream);
        }
#endif
#else
...
std::vector<at::Tensor> mha_bwd(
    ...
    std::optional<const at::Tensor>
        &seqused_k_, // b. If given, only this many elements of each batch
                     // element's keys are used.
    std::optional<int> max_seqlen_q_, std::optional<int> max_seqlen_k_,
    float const softmax_scale, bool is_causal, int window_size_left,
    int window_size_right, int const sink_token_length, float const softcap,
    bool const deterministic, int const sm_margin) {
  ...
  if (num_heads_k != num_heads && params.deterministic) {
    ...
    at::Tensor dk_semaphore = torch::empty(
        {(seqlen_k + kBlockN - 1) / kBlockN, batch_size, num_heads_k},
        opts.dtype(torch::kInt32));
    at::Tensor dv_semaphore = torch::empty(
        {(seqlen_k + kBlockN - 1) / kBlockN, batch_size, num_heads_k},
        opts.dtype(torch::kInt32));
...
mha_combine(const at::Tensor &out_partial, // num_splits x batch_size x seqlen x
                                           // num_heads x head_size
            const at::Tensor
                &lse_partial, // num_splits x batch_size x seqlen x num_heads
            std::optional<at::Tensor>
                out_, // batch_size x seqlen x num_heads x head_size
            std::optional<at::ScalarType> out_dtype_) {

  auto dprops = at::cuda::getCurrentDeviceProperties();
  bool is_sm8x = dprops->major >= 8;
  TORCH_CHECK(is_sm8x,
              "Attention combine function only supports Ampere GPUs or newer.");

  auto out_partial_type = out_partial.scalar_type();
  TORCH_CHECK(out_partial_type == at::ScalarType::Float,
              "Attention combine function only support fp32 data type");
  TORCH_CHECK(lse_partial.scalar_type() == at::ScalarType::Float,
              "Attention combine function only support fp32 data type");

  CHECK_DEVICE(out_partial);
  CHECK_DEVICE(lse_partial);

  TORCH_CHECK(out_partial.stride(-1) == 1,
              "Input tensor must have contiguous last dimension");
  TORCH_CHECK(lse_partial.stride(-2) == 1,
              "LSE tensor must be contiguous in the seqlen dimension");
  ...
  TORCH_CHECK(
      head_size_og <= 256,
      "FlashAttention combine only supports head dimension at most 256");
  TORCH_CHECK(num_splits <= 256,
              "FlashAttention combine only supports num_splits at most 256");

  CHECK_SHAPE(out_partial, num_splits, batch_size, seqlen, num_heads,
              head_size_og);
    ...
    return x.size(-1) % alignment == 0
               ...
               : torch::nn::functional::pad(
                     x, torch::nn::functional::PadFuncOptions(
                            {0, alignment - x.size(-1) % alignment}));
  };
  ...
  TORCH_CHECK(out_type == at::ScalarType::Float ||
                  out_type == at::ScalarType::BFloat16 ||
                  out_type == at::ScalarType::Half,
              "Output type must be FP32, FP16 or BF16");
  at::Tensor out;
  if (out_.has_value()) {
    out = out_.value();
    TORCH_CHECK(out.scalar_type() == out_type);
    ...
    if (head_size_og % alignment != 0) {
      out = torch::empty({batch_size, seqlen, num_heads, head_size},
                         opts.dtype(out_type));
    }
  } else {
    out = torch::empty({batch_size, seqlen, num_heads, head_size},
  ...
  auto softmax_lse =
      torch::empty({batch_size, num_heads, seqlen}, opts.dtype(at::kFloat))
          .transpose(1, 2);

  Flash_fwd_params
      params{}; // Need to reset the params to set everything to zero
