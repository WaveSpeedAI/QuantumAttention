  ...
  void operator()(Params const &params, char *smem_buf) {
    ...
    if (warp_group_idx == 0) { // Producer
      ...
      for (auto work_tile_info =
        ...
        collective_mainloop.load(params.mainloop, pipeline_k, pipeline_v,
                                 pipeline_vt, smem_pipe_write, shared_storage,
                                 scheduler_prefetch, seqlen_info, block_coord,
                                 work_idx);
      }
      collective_mainloop.load_tail(pipeline_k, pipeline_v, pipeline_vt,
                                    smem_pipe_write, shared_storage, work_idx);
    } else { // Consumer
      ...
      for (auto work_tile_info =
        ...
        flash::Softmax<2 * (2 * kBlockM / NumMmaThreads),
        ...
        bool tile_valid = collective_mainloop.mma(
            params.mainloop, pipeline_k, pipeline_v, smem_pipe_read, tOrO,
        ...
        if (tile_valid) {
          ...
          collective_epilogue.store(params.epilogue, tOrO, softmax.row_sum,
          ...
          collective_epilogue.template store_zero<!Split /*Clear_O*/>(
