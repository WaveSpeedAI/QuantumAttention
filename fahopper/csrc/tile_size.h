...
tile_size_fwd_sm90(int headdim, bool is_causal, bool is_local,
  ...
  if (element_size == 2) {
    if (headdim <= 64) {
      ...
      return {128, paged_kv || is_local ? 96 : 112, true,
    ...
    if (headdim <= 64) {
      ...
      return {128, is_local ? 64 : 128, true,
              !paged_kv}; // PagedKV uses more registers so we disabled
                          // IntraWGOverlap
    }
  }
}
...
tile_size_fwd_sm8x(bool sm86_or_89, int headdim, bool is_causal, bool is_local,
                   int element_size = 2, bool paged_kv = false,
