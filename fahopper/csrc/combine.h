...
// template<typename Kernel_traits, int kBlockM, int Log_max_splits, bool
// Is_even_K, typename Params>
template <typename Element, typename ElementAccum, int kHeadDim, int kBlockM,
          int Log_max_splits, bool Is_even_K, typename Params>
__global__ void combine_attn_seqk_parallel(Params const params) {
  ...
  using SharedStorage =
      SharedStorageLSE<ElementAccum, Shape<Int<kMaxSplits>, Int<kBlockM + 1>>,
                       Shape<Int<kMaxSplits>>>;
  SharedStorage &shared_storage = *reinterpret_cast<SharedStorage *>(smem_);
  Tensor sLSE = make_tensor(make_smem_ptr(shared_storage.smem_lse.data()),
                            Shape<Int<kMaxSplits>, Int<kBlockM + 1>>{});
  Tensor sValidSplits =
      make_tensor(make_smem_ptr(shared_storage.smem_valid_splits.data()),
                  Shape<Int<kMaxSplits>>{});
  ...
  // params.seqlen_q, params.b * params.h * params.seqlen_q);
...
#pragma unroll
  ...
  // Reduce along the kBlockM dimension to determine valid splits (store in
  // SMEM) One thread per split. Know NumThreads = 128 >= NumMaxSplits
  if (tidx < kMaxSplits) {
...
#pragma unroll
  for (int l = 0; l < kNLsePerThread; ++l) {
    ...
    // if (bidx == 0 && tidx < 128) { printf("tidx = %d, row = %d, col = %d, lse
...
#pragma unroll
        ...
        // sLSE(split, 1)); print_tensor(tOrOaccum); }
      }
    }
...
#pragma unroll
  for (int m = 0; m < size<1>(rO); ++m) {
    ...
    // params.h, params.seqlen_q, params.b * params.h * params.seqlen_q);
