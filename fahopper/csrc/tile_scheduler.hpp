...
public:
  ...
  static Params to_underlying_arguments(TileSchedulerArguments const &args) {
    int const size_one_kv_head =
        args.seqlen_k * args.headdim * args.element_size * 2;
...
template <int kBlock, int NumMmaThreads = 2 * cutlass::NumThreadsPerWarpGroup,
          int NumProducerThreads = cutlass::NumThreadsPerWarp,
          bool Split = false, bool PackGQA = false, bool WarpSpecialized = true>
...
public:
  ...
  static Params to_underlying_arguments(TileSchedulerArguments const &args) {
    ...
    assert(args.tile_count_semaphore != nullptr);
    return {args.num_head * (!Split ? 1 : args.num_splits),
            args.num_batch,
            args.qhead_per_khead,
            args.seqlen,
            cutlass::FastDivmod(!Split ? 1 : args.num_splits),
  ...
  WorkTileInfo tile_idx_to_work_tile(Params const &params, int next_tile_idx,
    ...
    auto get_num_m_blocks = [&](int bidb_start) {
      ...
      int seqlen;
      if (params.seqused) {
        seqlen = lane + bidb_start < params.num_batch
                     ? params.seqused[lane + bidb_start]
                     : 0;
      } else if (params.cu_seqlens) {
        int cur_cu_seqlen = lane + bidb_start <= params.num_batch
                                ? params.cu_seqlens[lane + bidb_start]
                                : 0;
    ...
    int group_end_tile =
        current_work.tile_idx - current_work.block -
