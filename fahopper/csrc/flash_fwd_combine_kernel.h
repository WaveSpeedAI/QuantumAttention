...
public:
  ...
  using SmemLayoutO =
      Layout<Shape<Int<kBlockM>, Int<kHeadDim>, Int<kStages>>,
...
#pragma unroll
    ...
    Tensor mOpartial = make_tensor(
        ...
        params.stride_O_partial); // (seqlen, d, num_splits, head, batch)
...
#pragma unroll
    for (int m = 0; m < size<1>(tOcO); ++m) {
      ...
      int idx = m_block * kBlockM + mi;
      if constexpr (!Varlen) {
        tObidb[m] = params.head_divmod.divmod(
...
#pragma unroll
      for (int k = 0; k < size(tOpO); ++k) {
        tOpO(k) = get<1>(tOcO(_0{}, _0{}, k)) < get<1>(params.shape_O_partial);
      }
    }
      ...
      for (int m = 0; m < size<1>(tOcO); ++m) {
        if (tObidb(m) >= 0) {
          Tensor mOpartial_cur =
              make_tensor(make_gmem_ptr(tOrOptr[m]),
                          mOpartial(_0{}, _, _, _0{}, _0{}).layout());
...
#pragma unroll
    ...
    Tensor mO = make_tensor(
        make_gmem_ptr(params.ptr_O + offset * get<0>(params.stride_O)), shape_O,
        params.stride_O);
    Tensor mO_copy =
        ...
        for (int k = 0; k < size<2>(tOcO); ++k) {
          ...
          if (Is_even_K || tOpO(k)) {
            cute::copy(gmem_tiled_copy_O, rO(_, m, k),
                       mO_copy(_, tOmidx(m), k_idx, tObidh(m), tObidb(m)));
          }
        }
      }
    }
